...
if ! `az group exists --name ${AKS_RESOURCE_GROUP}`; then
   echo "Creating the Resource Group (${AKS_RESOURCE_GROUP})..."
...
#
echo "Creating namespace (${AKS_NAMESPACE}) within the cluster (${AKS_CLUSTER_NAME})..."
...
echo "Creating nodepool (${AKS_NODEPOOL_NAME}) within the cluster (${AKS_CLUSTER_NAME})..."
...
sleep 5
helm upgrade -i neo4j-core-3  neo4j/neo4j --namespace ${AKS_NAMESPACE} -f hybrid-core-small.yaml
sleep 5
helm upgrade -i neo4j-gds-1  neo4j/neo4j --namespace ${AKS_NAMESPACE} -f hybrid-gds-small.yaml 

#
# Create load balancers
#
echo "Creating load balancers..."
kubectl apply -f neo4j-core-lb.yaml
kubectl apply -f neo4j-gds-lb.yaml
